// KAT (Known Answer Test) vectors for SHA-256
// Source: NIST FIPS 180-4 / NIST CAVP — test-vectors/sha-2/sha256-acvp.json
...
#[test]
fn sha256_kat_vectors() {
    let path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../test-vectors/sha-2/sha256-acvp.json"
    );
    ...
    for tv in &file.sample_test_vectors {
        let msg = match hex::decode(&tv.msg) {
            Ok(m) => m,
            Err(_) => continue, // skip non-hex entries (e.g. 1M-byte placeholder)
        };
        let expected = hex::decode(&tv.md).unwrap();
        let got = metamui_sha2::sha256::sha256(&msg);
        assert_eq!(
            got.as_ref(),
            expected.as_slice(),
            "SHA-256 KAT failed at tc_id={}",
            tv.id
        );
        count += 1;
    }
    println!("SHA-256: {count} KAT vectors passed");
    assert!(count >= 3, "expected at least 3 vectors, got {count}");
}
