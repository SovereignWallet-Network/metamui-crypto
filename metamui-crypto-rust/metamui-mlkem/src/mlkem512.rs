//! ML-KEM-512 implementation (NIST Level 1 security)
...
impl PublicKey {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        if bytes.len() != MLKEM512_PARAMS.public_key_bytes {
...
impl SecretKey {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        if bytes.len() != MLKEM512_PARAMS.secret_key_bytes {
            return Err(MLKemError::InvalidKeySize);
        }
...
impl Ciphertext {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        if bytes.len() != MLKEM512_PARAMS.ciphertext_bytes {
...
/// Generate an ML-KEM-512 keypair from explicit (d, z) per FIPS 203 §6.1.
///
...
pub fn generate_keypair_from_dz(d: &[u8; 32], z: &[u8; 32]) -> Result<(PublicKey, SecretKey)> {
    let mut pk = PublicKey { bytes: [0u8; 800] };
    let mut sk = SecretKey { bytes: [0u8; 1632] };
    crate::kem::mlkem_keygen_from_dz(&MLKEM512_PARAMS, &mut pk.bytes, &mut sk.bytes, d, z)?;
    Ok((pk, sk))
}

/// Generate an ML-KEM-512 keypair deterministically from a 32-byte seed.
///
...
pub fn generate_keypair_from_seed(seed: &[u8; 32]) -> Result<(PublicKey, SecretKey)> {
    let mut pk = PublicKey { bytes: [0u8; 800] };
    let mut sk = SecretKey { bytes: [0u8; 1632] };
    crate::kem::mlkem_keygen_from_seed(&MLKEM512_PARAMS, &mut pk.bytes, &mut sk.bytes, seed)?;
    Ok((pk, sk))
}

/// Deterministically encapsulate an ML-KEM-512 shared secret using `m` as
...
pub fn encapsulate_deterministic(
    ...
    let mut ct = Ciphertext { bytes: [0u8; 768] };
...
/// Decapsulate an ML-KEM-512 shared secret (free-function convenience
...
impl Kem for MLKem512 {
    ...
    fn generate_keypair<R: RngCore + CryptoRng>(
        ...
        mlkem_keygen(&MLKEM512_PARAMS, &mut pk.bytes, &mut sk.bytes, rng)?;
...
mod tests {
    ...
    fn test_mlkem512_keygen() {
        ...
        let result = MLKem512::generate_keypair(&mut rng);
        assert!(result.is_ok());
        
        let (pk, sk) = result.unwrap();
        assert_eq!(pk.as_bytes().len(), 800);
        assert_eq!(sk.as_bytes().len(), 1632);
    }
    
    #[test]
    fn test_mlkem512_encap_decap() {
        let mut rng = thread_rng();
        
        // Generate keypair
        let (pk, sk) = MLKem512::generate_keypair(&mut rng).unwrap();
        
        // Encapsulate
        let (ct, ss1) = MLKem512::encapsulate(&pk, &mut rng).unwrap();
    ...
    #[test]
    fn test_mlkem512_basic() {
        test_mlkem512_keygen();
        test_mlkem512_encap_decap();
    }

    #[test]
    fn test_mlkem512_deterministic_roundtrip() {
        let seed = [0x42u8; 32];
        let m = [0x77u8; 32];
    ...
    #[test]
    fn test_mlkem512_deterministic_is_deterministic() {
        let seed = [0x55u8; 32];
        let m = [0x78u8; 32];
...
/// FIPS 203 §7.2 encapsulation-key check for ML-KEM-512 (see
/// [`crate::kem::mlkem_validate_encapsulation_key`]).
pub fn validate_encapsulation_key(ek: &[u8]) -> bool {
    crate::kem::mlkem_validate_encapsulation_key(&crate::params::MLKEM512_PARAMS, ek)
}

/// FIPS 203 §7.3 decapsulation-key check for ML-KEM-512 (see
/// [`crate::kem::mlkem_validate_decapsulation_key`]).
pub fn validate_decapsulation_key(dk: &[u8]) -> bool {
    crate::kem::mlkem_validate_decapsulation_key(&crate::params::MLKEM512_PARAMS, dk)
}
