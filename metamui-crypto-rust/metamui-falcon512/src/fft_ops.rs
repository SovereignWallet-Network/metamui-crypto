// MetaMUI Falcon - FFT domain operations (f64)
//
// Copyright (c) 2025 Sovereign Wallet Co., Ltd. All Rights Reserved.
// SPDX-License-Identifier: Apache-2.0

//! Complex FFT domain operations for the signing pipeline.
//!
...
//! assembly or GPU path, so a signature is computed by the same arithmetic
//! wherever it is produced.
//!
//! # Backend hook
//!
//! Every operation the signing pipeline uses goes through the [`FftBackend`]
//! that [`backend`] returns. The default is [`SCALAR`], the implementations in
//! [`scalar`]. A separate crate may install another backend once, with
//! [`install`], before the first Falcon operation of the process; after that
//! the choice is fixed and `install` returns `Err`. A backend must compute
//! the same values as [`scalar`] (it is an implementation of the same
//! arithmetic, not a different algorithm): the signature bytes depend on it.
//! This crate ships no other backend.

use std::sync::OnceLock;

/// The scalar implementations, the default backend and the oracle any other
...
/// A set of FFT-domain operations on the SOA layout.
///
/// Every field has the signature of the [`scalar`] function of the same name.
#[derive(Clone, Copy)]
pub struct FftBackend {
    /// A short name for diagnostics, e.g. `"scalar"`.
    pub name: &'static str,
    pub poly_add_fft: fn(&mut [f64], &[f64], &[f64], usize),
    pub poly_sub_fft: fn(&mut [f64], &[f64], &[f64], usize),
    pub poly_mul_fft: fn(&mut [f64], &[f64], &[f64], usize),
    pub poly_muladj_fft: fn(&mut [f64], &[f64], &[f64], usize),
    pub poly_adj_fft: fn(&mut [f64], &[f64], usize),
    pub poly_inv_fft: fn(&mut [f64], &[f64], usize),
    pub poly_norm_fft: fn(&mut [f64], &[f64], usize),
    pub poly_muladd_fft: fn(&mut [f64], &[f64], &[f64], &[f64], usize),
    pub poly_mulsub_fft: fn(&mut [f64], &[f64], &[f64], &[f64], usize),
    pub poly_split_fft: fn(&mut [f64], &mut [f64], &[f64], usize),
    pub poly_merge_fft: fn(&mut [f64], &[f64], &[f64], usize),
    pub fft_forward: fn(&mut [f64], usize),
    pub fft_inverse: fn(&mut [f64], usize),
}

impl core::fmt::Debug for FftBackend {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("FftBackend").field("name", &self.name).finish()
    }
}

/// The scalar backend: [`scalar`], field by field.
pub static SCALAR: FftBackend = FftBackend {
    name: "scalar",
    poly_add_fft: scalar::poly_add_fft,
    poly_sub_fft: scalar::poly_sub_fft,
    poly_mul_fft: scalar::poly_mul_fft,
    poly_muladj_fft: scalar::poly_muladj_fft,
    poly_adj_fft: scalar::poly_adj_fft,
    poly_inv_fft: scalar::poly_inv_fft,
    poly_norm_fft: scalar::poly_norm_fft,
    poly_muladd_fft: scalar::poly_muladd_fft,
    poly_mulsub_fft: scalar::poly_mulsub_fft,
    poly_split_fft: scalar::poly_split_fft,
    poly_merge_fft: scalar::poly_merge_fft,
    fft_forward: scalar::fft_forward,
    fft_inverse: scalar::fft_inverse,
};

static BACKEND: OnceLock<&'static FftBackend> = OnceLock::new();

/// The backend in use: what [`install`] set, or [`SCALAR`].
///
/// The first call fixes the choice for the rest of the process.
#[inline]
pub fn backend() -> &'static FftBackend {
    BACKEND.get_or_init(|| &SCALAR)
}

/// Install `b` as the process-wide backend.
///
/// Succeeds once, and only before the first call of [`backend`] (which every
/// Falcon signing operation makes). Afterwards it returns `Err` with the
/// backend that is in use, and that one stays: a signature must not depend on
/// which call came first.
pub fn install(b: &'static FftBackend) -> Result<(), &'static FftBackend> {
    match BACKEND.set(b) {
        Ok(()) => Ok(()),
        Err(_) => Err(backend()),
    }
...
macro_rules! via_backend {
    ($(fn $name:ident($($arg:ident: $ty:ty),*);)*) => {$(
        #[doc = concat!("`", stringify!($name), "` of the installed backend (see [`backend`]).")]
        #[inline]
        pub fn $name($($arg: $ty),*) {
            (backend().$name)($($arg),*)
        }
...
via_backend! {
    fn poly_add_fft(c: &mut [f64], a: &[f64], b: &[f64], logn: usize);
    fn poly_sub_fft(c: &mut [f64], a: &[f64], b: &[f64], logn: usize);
    fn poly_mul_fft(c: &mut [f64], a: &[f64], b: &[f64], logn: usize);
    fn poly_muladj_fft(c: &mut [f64], a: &[f64], b: &[f64], logn: usize);
    fn poly_adj_fft(c: &mut [f64], a: &[f64], logn: usize);
    fn poly_inv_fft(c: &mut [f64], a: &[f64], logn: usize);
    fn poly_norm_fft(c: &mut [f64], a: &[f64], logn: usize);
    fn poly_muladd_fft(c: &mut [f64], a: &[f64], b: &[f64], acc: &[f64], logn: usize);
    fn poly_mulsub_fft(c: &mut [f64], a: &[f64], b: &[f64], acc: &[f64], logn: usize);
    fn poly_split_fft(f0: &mut [f64], f1: &mut [f64], f: &[f64], logn: usize);
    fn poly_merge_fft(f: &mut [f64], f0: &[f64], f1: &[f64], logn: usize);
    fn fft_forward(f: &mut [f64], logn: usize);
    fn fft_inverse(f: &mut [f64], logn: usize);
}
...
mod backend_tests {
    ...
    #[test]
    fn scalar_is_the_default_and_install_is_refused_after_first_use() {
        // Any earlier test in this binary may already have fixed the backend;
        // either way it is the scalar one, because nothing here installs another.
        assert_eq!(backend().name, "scalar");
        let other: &'static FftBackend = Box::leak(Box::new(FftBackend { name: "other", ..SCALAR }));
        assert_eq!(install(other).unwrap_err().name, "scalar");
        assert_eq!(backend().name, "scalar");
    }

    #[test]
    fn wrappers_reach_the_scalar_functions() {
        let logn = 3;
        let n = 1usize << logn;
        ...
        poly_mul_fft(&mut c1, &a, &b, logn);
        scalar::poly_mul_fft(&mut c2, &a, &b, logn);
        assert_eq!(c1, c2);
        let mut f = a.clone();
        fft_forward(&mut f, logn);
        fft_inverse(&mut f, logn);
        for i in 0..n {
            assert!((f[i] - a[i]).abs() < 1e-9);
        }
    }
}
